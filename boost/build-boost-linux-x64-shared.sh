...
pushd ${gitdir}/boost
./bootstrap.sh --without-libraries=python 1>>${logfile} 2>>${logfile}
...
./b2 install --prefix=${installdir}/${gittag} --exec-prefix=${installdir}/${gittag} link=shared address-model=64 threading=multi --variant=debug,release 1>>${logfile} 2>>${logfile}
popd
...
echo "  BOOST_INCLUDEDIR=${rootdir}/include_nover"
